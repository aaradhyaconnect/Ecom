...
export default function WishlistPage() {
  ...
  const handleAddToCart = (product: Product) => {
    ...
    addItem(product, 1, size, colors[0]?.name || "Default");
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4">
        ...
        <h1 className="text-2xl font-serif font-bold text-charcoal mb-2">Your wishlist is empty</h1>
        <p className="text-charcoal-muted mb-8">Save your favorite items here</p>
        <Link href="/products/new-arrivals">
          <Button variant="outline" size="lg"><ArrowLeft className="h-4 w-4 mr-2" />Explore Products</Button>
        </Link>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="text-2xl lg:text-3xl font-serif font-bold text-charcoal mt-1">My Wishlist ({items.length})</h1>
        </div>
        <Link href="/products/new-arrivals" className="text-xs uppercase tracking-[0.2em] text-charcoal-muted hover:text-charcoal transition-colors">Continue Shopping</Link>
      </div>
            ...
            <div key={product.id} className="group relative">
              <Link href={`/product/${product.slug}`} className="block">
                <div className="relative aspect-[3/4] bg-charcoal/5 mb-3">
                  <Image src={product.images?.[0] || "/placeholder.svg"} alt={product.name} fill sizes="(max-width: 768px) 50vw, (max-width: 1024px) 33vw, 25vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
                  <div className="absolute top-2 left-2 flex flex-col gap-1">
                    {product.is_new && <Badge variant="new">New</Badge>}
                    {product.is_sale && discount > 0 && <Badge variant="sale">-{discount}%</Badge>}
                  </div>
                  <button onClick={(e) => { e.preventDefault(); removeItem(product.id); toast.success("Removed from wishlist"); }} className="absolute top-2 right-2 p-2 bg-ivory/80 backdrop-blur-sm hover:scale-110 transition-all">
              ...
              <Button variant={inCart ? "secondary" : "primary"} size="sm" fullWidth className="mt-3" onClick={() => handleAddToCart(product)} disabled={inCart}>
                <ShoppingBag className="h-3.5 w-3.5 mr-1.5" />{inCart ? "In Cart" : "Add to Cart"}
              </Button>
            </div>
          );
        })}
